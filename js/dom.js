...
function createForm() {
  ...
  labelTitle.appendChild(
    document.createTextNode("Masukan Hal yang akan Dilakukan")
  );

  // label for date
  let labelDate = document.createElement("label");
  labelDate.setAttribute("for", "date");
  labelDate.appendChild(
    document.createTextNode("Masukkan tanggal harus selesai")
  );
...
function makeTodo(data, timestamp, isCompleted) {
  ...
  if (isCompleted) {
    ...
    container.append(createEditButton(), createCheckButton());
  }
...
function editTask(taskElement) {
  ...
  const taskTitle1 = taskElement.querySelector(".inner > h2").id;
  const taskTimestamp2 = taskElement.querySelector(".inner > p").id;
  ...
  inputDate.value = taskTimestamp;

  editTitleId = taskTitle1;
  editDateId = taskTimestamp2;
